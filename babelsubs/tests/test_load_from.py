...
class LoadFromTest(TestCase):
    def test_type_specified(self):
        subs  = load_from(utils.get_data_file_path("simple.srt"), type='srt')
        parsed = subs.to_internal()
        self.assertEquals(len(parsed), 19)

    def test_type_guessing(self):
        subs  = load_from(utils.get_data_file_path("simple.srt"))
        parsed = subs.to_internal()
        self.assertEquals(len(parsed), 19)
        
    def test_type_override(self):
        subs  = load_from(utils.get_data_file_path("simple-srt.badextension"), type='srt')
        parsed = subs.to_internal()
        self.assertEquals(len(parsed), 19)
